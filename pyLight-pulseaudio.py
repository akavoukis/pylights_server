...
class PeakMonitor(object):
    ...
    def stream_read_cb(self, stream, length, index_incr):
        ...
        for i in xrange(length):
            ...
            pre_amplified_data = data[i] -128
            self._samples.put(pre_amplified_data)
        pa_stream_drop(stream)

# ignore main threads
def my_clients_count(ignore=2):
...
def main():
    ...
    logging.basicConfig(filemode='w', level=int(args.log))
    port = int(args.port)
    active_connections = 0

    logging.info("Service pyLights Started, on port %d" %port)
    ...
    try:
        while True:
            connected_clients = my_clients_count()
            if connected_clients > 0 :
                for sample in monitor:
                    #sample = sample >> DISPLAY_SCALE
                    logging.debug(sample)
                    sample = (sample /100*AMPLIFY_PERCENT) + sample
                    level = sample
                    connected_clients = my_clients_count()
                    print ("connected clients %d" %connected_clients)
        ...
        server_thread.kill_received = True
